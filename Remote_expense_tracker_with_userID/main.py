...
def init_db():  # Keep as sync for initialization
    try:
        ...
        with sqlite3.connect(DB_PATH) as c:
            ...
            """)
            # Test write access
            c.execute("INSERT OR IGNORE INTO expenses(userID, date, amount, category) VALUES ('test_user', '2000-01-01', 0, 'test')")
...
@mcp.tool()
async def add_expense(userID,date, amount, category, subcategory="", note=""):  # Changed: added async
    '''Add a new expense entry to the database.'''
    try:
        async with aiosqlite.connect(DB_PATH) as c:  # Changed: added async
            cur = await c.execute(  # Changed: added await
                "INSERT INTO expenses(userID, date, amount, category, subcategory, note) VALUES (?,?,?,?,?,?)",
                (userID,date, amount, category, subcategory, note)
            )
...
@mcp.tool()
async def list_expenses(userID, start_date, end_date):  # Changed: added async
...
@mcp.tool()
async def delete_expense(expense_id: int,userID: str):
...
@mcp.tool()
async def summarize(userID, start_date, end_date, category=None):  # Changed: added async
